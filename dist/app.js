...
async function searchDocuments(query, collectionName = "3GPP_DATA", limit = 3) {
    try {
...
async function Grade(document, question) {
    // console.time("ExecutionTime");
    const gradeSystem = `
...
async function relevantImages(query) {
    // console.time("ExecutionTime");
...
async function rephrasedQuery(state) {
...
async function gradeDocuments(state) {
    ...
    const gradedDocs = await Promise.all(documents.map(async (doc) => ({
        doc,
...
class GeneralQuestion {
    ...
    constructor(binary_score) {
        if (!["yes", "no"].includes(binary_score.toLowerCase())) {
            throw new Error("Invalid binary_score: must be 'yes' or 'no'. Received: " + binary_score);
        }
        this.binary_score = binary_score.toLowerCase();
    }
}
// Function to classify and answer questions
async function generalQA(query) {
...
//   }
// }
async function runGraph(query, id) {
    const config = {
        configurable: { thread_id: id },
    };
    try {
        ...
        //  const source_information = 
        const source_information = (result.source_info?.length > 0 ?
            `${result.source_info[0]}\n\n` + // First document info
                `See other relevant documents:` + "\n\n" +
